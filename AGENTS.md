...
## Session Summary — Onboarding Receipt Buttons + Installment Field Locking (Complete)
...
- **Uncommitted** (working tree): `backend/app/services/payment.py`, `backend/app/services/consultation.py`, `backend/app/api/v1/receipts.py`, `frontend/src/app/features/clients/clients.{ts,html}`, `frontend/src/app/features/clients/client-profile.{ts,html}`; untracked `frontend/debug8.log` (pre-existing), `frontend/e2e/onboarding-receipt.spec.ts` (new regression test).
