...
- **Branch-scoped default views**: Non-privileged users now see only their assigned branch's consultations by default (607 vs 622 total when one branch assigned). Search allows cross-branch results within the company. Added `resolve_branch_ids()` helper in `tenant.py`; updated consultations list, web dashboard, expenses, collections, borrowed, transfers endpoints to auto-resolve branches. Changed `search_consultations` INNER JOIN to outerjoin for null-branch consultations. Fixed empty-string search bypassing branch filter (`has_search = bool(search and search.strip())`).
