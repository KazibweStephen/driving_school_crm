...
async def resolve_branch_ids(
    ...
    - Only ``super_user`` may see all company branches (or their own company's
      branches when company-scoped).
    - Every other user (including office_admin, manager, branch_supervisor) is
      limited to their assigned branches via ``UserBranchAssignment``.
      Requested IDs must be a subset of assigned; if none requested, the
      assigned branches are returned.
    """
