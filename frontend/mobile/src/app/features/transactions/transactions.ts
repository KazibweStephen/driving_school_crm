...
export class Transactions {
  ...
  clientType = 'all';
  totals = signal<PaymentTotals>({ total_amount_sum: '0', total_paid_sum: '0', total_balance_sum: '0' });
  ...
  hasMore = computed(() => this.payments().length < this.total());
  ...
  loadPayments() {
    ...
    this.paymentService
      ...
      .subscribe({
        next: (res) => {
          this.payments.set(this.page() === 1 ? res.payments : [...this.payments(), ...res.payments]);
        ...
        error: () => {
          this.loading.set(false);
          this.messageService.add({ severity: 'error', summary: 'Could not load transactions' });
        },
      });
  }
  ...
  statusLabel(p: PaymentWithClient): string {
    const bal = parseFloat(String(p.balance));
    if (bal <= 0) return 'Paid';
    if (bal >= parseFloat(String(p.total_amount))) return 'Unpaid';
    return 'Partial';
  }

  statusClass(p: PaymentWithClient): string {
    const bal = parseFloat(String(p.balance));
    if (bal <= 0) return 'bg-green-100 text-green-700';
    if (bal >= parseFloat(String(p.total_amount))) return 'bg-red-100 text-red-700';
